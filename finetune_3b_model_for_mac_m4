...
if DATA_FILE == "":
    ...
    if json_files:
        ...
        print(f" No training files found, using default: {DATA_FILE}")
...
if os.path.exists(PREVIOUS_DIR):
    print(f"\n [INFO] Previous training found in {PREVIOUS_DIR}")
...
print(f"\n [INFO] Gradient checkpointing enabled (saves ~40% memory)")
...
if num_examples < 100:
    ...
    print(f"   For A100 training: 5-35 epochs is acceptable")
    print(f"   Alpha 64 = 4x stronger than base model (prevents hallucination)")
else:
    lora_rank = 32
